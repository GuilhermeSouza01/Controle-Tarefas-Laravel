                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($tarefas as $key => $tf)
                                <tr>
                                    <th scope="row">{{$tf['id']}}</th>
                                    <td>{{$tf['tarefa']}}</td>
                                    <td>{{ date('d/m/Y', strtotime($tf['data_limite_conclusao'])) }}</td>
                                    <td><a href="{{route('tarefa.edit', $tf['id'])}}">Editar</a></td>
                                    <td>
                                        <form id="form_{{$tf['id']}}" method="post" action="{{ route('tarefa.destroy', ['tarefa' => $tf['id']]) }}">
                                            @method('DELETE')
                                            @csrf
                                        </form>
                                        <a href="#" onclick="document.getElementById('form_{{$tf['id']}}').submit()">Excluir</a>
                                    </td>
                                </tr>
                                @endforeach
